function [X, Y, Z, Coeff] = spotfinder_3D(filename,varargin)
...
for m=1:length(B)
    ...
    X(k)=double(Px)+Xc-double((windowsize+1)/2); %#ok<*AGROW>
    Y(k)=double(Py)+Yc-double((windowsize+1)/2);
    ...
    for l=1:zstacks
        ...
        stack=double(imread(strcat(stackfol,'\',stackfiles(l).name)));
    ...
    temp = sort(TopZr);
    TopZ=TopZr-mean(temp(1:floor(3*length(temp)/4)));
    Z(k)=sum((1:zstacks).*TopZ)/sum(TopZ);
    k = k+1;
end
